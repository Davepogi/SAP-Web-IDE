			...
			onTheMasterPage : {
				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("WiId") === oObject.WiId;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].WiId;
										} else {
											sCurrentId = aItemsNotInTheList[0].WiId;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("CostHdrSet", {
											WiId : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("WiId"),
							title: oBindingContext.getProperty("WiText")
						};
					}
				},
				...
				assertions : {
					...
					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("WiText");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all CostHdrSet to be able to check for the total amount
